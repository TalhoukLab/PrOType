...
predict_study <- function(fit, new.dat)
#********************************************************************
# Predict on a test set (new.dat) from a fitted model returned from
# train()
#********************************************************************
{
  x <- new.dat[,-1]
  lab <- new.dat[,1]
  pred <- splendid::prediction(mod = fit, data = x, class = lab, threshold = 0)
  
  return(pred)
}

eval <- function(pred, true, train.name, test.name)
#********************************************************************
# Return list of evaluation measures and confusion matrices. This
# output is required for plots.
#********************************************************************
{
  lab.index <- data.frame(pred, index = as.integer(pred))
  lab.map.tab <- group_by(lab.index, pred) %>% dplyr::summarise(index = mean(index))
  relab <- diceR::relabel_class(true, lab.index$index) %>%
    data.frame(index = .) %>%
    inner_join(., lab.map.tab, by = "index") %>%
    select(pred) %>% .$pred
  
  pred1 <- as.factor(pred) %>% as.integer()
  relab <- diceR::relabel_class(true, pred1) %>% as.factor()
  levels(relab) <- levels(pred)
  
  evals <- splendid::evaluation(as.factor(relab), pred, plot = FALSE)
  
  return(list(
    evals, 
    caret::confusionMatrix(relab, pred)
  ))
}
