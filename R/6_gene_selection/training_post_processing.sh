...
file_to_submit=("R/6_gene_selection/build_final_model.sh")
...
echo 'source("R/6_gene_selection/evaluate_predictions.R")' >> $R_train
...
echo 'shouldCompute <- '$shouldCompute >> $R_train
echo 'outputDir <- "'$outputDir'"' >> $R_train
echo 'source("R/6_gene_selection/summary_frequencies.R")' >> $R_train
