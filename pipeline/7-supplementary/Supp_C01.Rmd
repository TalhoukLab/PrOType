...
otta_2015 <-
  readr::read_csv(file.path(params$outputDir, "gene_selection/final_model/Final_Predictions.csv"),
                  col_types = list("X1" = readr::col_skip())) %>%
  dplyr::select(ottaID, pred_2015 = final)
otta_2017 <-
  readr::read_csv(here("data/nstring/nano2_ns_predictions.csv")) %>%
  dplyr::transmute(
...
otta_2018 <-
  readr::read_csv(here("data/nstring/nano3_ns_predictions.csv")) %>%
  dplyr::transmute(
