...
pred <- readr::read_csv(here("data/predictions.csv"))
...
fig_path <- "pipeline/7-supplementary/figures/C03"
```
...
# Check otta IDs
stopifnot(identical(pred[["ottaID"]], clin_d[["ottaID"]]))
...
```{r km_final_all, fig.height=10, fig.width=7}
...
ggsave(here(fig_path, "km_final_all.pdf"), plots_km_final_all, width = 7, height = 10)
```

```{r km_final_adnexal, fig.height=10, fig.width=7}
...
ggsave(here(fig_path, "km_final_adnexal.pdf"), plots_km_final_adnexal, width = 7, height = 10)
```
