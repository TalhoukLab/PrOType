...
load(here("data/expCS.rda"))
load(here("data/cs3.norm.rda"))

fig_path <- "pipeline/7-supplementary/figures/B06"

# Load otta datasets: CodeSet 3 gene expression and annotation data
data(rawOTTA, annot, package = "otta")
cs3 <- rawOTTA
exp0 <- annot %>%
...
exp_CS3_all <- exp0 %>%
  ...
  nanostringr::NanoStringQC(raw = cs3, exp = ., detect = 50, sn = 170)
```
...
```{r qc_filter}
# Final QC table with ARL samples removed (and keep only PrOType)
arl_dat <-
...
print(p)
ggsave(here(fig_path, "snr_vs_pergd.pdf"), p, width = 7, height = 5)
```
...
print(p)
ggsave(here(fig_path, "pools_gx_time.pdf"), p, width = 7, height = 5)
```
...
print(p)
ggsave(here(fig_path, "cc_hist.pdf"), p, width = 7, height = 5)
```
